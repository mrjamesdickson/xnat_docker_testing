...
(function(root, factory){
    ...
    return function(config){
        ...
        function initConfigDefaults(){
            if (config.escapeMode === undef) {
                config.escapeMode = true;
            }
            if (config.escapeOnlyBrackets !== undef) {
                 config.escapeMode = true;
            }
            if (config.attributePrefix === undef) {
                config.attributePrefix = "_";
            }
            config.arrayAccessForm = config.arrayAccessForm || "none";
            config.emptyNodeForm = config.emptyNodeForm || "text";

            if (config.enableToStringFunc === undef) {
                config.enableToStringFunc = true;
            }
            config.arrayAccessFormPaths = config.arrayAccessFormPaths || [];
            if (config.skipEmptyTextNodesForObj === undef) {
                config.skipEmptyTextNodesForObj = true;
            }
            if (config.stripWhitespaces === undef) {
                config.stripWhitespaces = true;
            }
            config.datetimeAccessFormPaths = config.datetimeAccessFormPaths || [];

            if (config.useDoubleQuotes === undef) {
        ...
        function escapeXmlChars(str){
            if (typeof(str) === "string") {
                if (config.escapeOnlyBrackets === true) {
                    return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');  // basal: fix weblication style
                }
                else {
        ...
        function parseDOMChildren(node, path){
            var result, nodeChildren, cidx, child, childName;
            if (node.nodeType === DOMNodeTypes.DOCUMENT_NODE) {
                ...
                else if (result.__cnt > 1 && result.__text != null && /^[-+]?[0-9]*\.?[0-9]+$/.test(result.__text) && config.parseNumbers) {
        ...
        function parseJSONObject(jsonObj, jsonObjPath){
            ...
            if (elementsCnt > 0) {
                for (var it in jsonObj) {
                    if (!jsonObj.hasOwnProperty(it)) continue;
        ...
        this.parseXmlStringX = function(xmlDocStr){
