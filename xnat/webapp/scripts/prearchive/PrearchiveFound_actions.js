/*
 * web: PrearchiveFound_actions.js
...
XNAT.app.prearchiveActions={
	...
	requestMoveDialog:function(oldProj, timestamp, folder){
		$('#oldProjString').val(oldProj);
		var formattedTimestamp = timestamp.replace(" ","_").replace("-","").replace("-","").replace(":","").replace(":","").replace(".","");
		while(formattedTimestamp.length<18){
			formattedTimestamp+="0"
		}
		$('#timestampString').val(formattedTimestamp);
		$('#folderString').val(folder);
	...
	move:function(proj){
		...
		this.moveCallback={
            ...
            failure:function(o){
        		...
        		if(o.status==301 || o.status==0){
            		window.location=serverRoot+"/REST/prearchive/projects/"+ proj + "/"+ $('#timestampString').val() +"/" + $('#folderString').val()+"?format=html&screen=PrearchiveDetails.vm&popup=false";
        		}else{
        ...
        YAHOO.util.Connect.asyncRequest('POST',serverRoot+"/REST/prearchive/projects/"+$('#oldProjString').val()+"/"+ $('#timestampString').val() +"/" + $('#folderString').val()+"?action=move&newProject=" + proj +"&XNAT_CSRF=" + csrfToken,this.moveCallback,null,this);
	},
	loadLogs:function(){
		var logsCallback={
	        success:function(o){
