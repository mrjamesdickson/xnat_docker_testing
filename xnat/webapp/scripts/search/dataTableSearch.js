...
XNAT.app.tableSizer = function(){
    ...
    var divWidth = $('#layout_content').width();
    /* what happens when there are more than one of these on a page? */
    var tableWidth = $('table.x_rs_t').width();
    /* what happens if there are more than one of these on a page? */
...
//	YAHOO.util.Cookie.remove(this.div_table_id + ".initialRequest");
    ...
    this.getPage = function(page){
        ...
        //xmodal.loading.open({title:'Loading data...'});
    ...
    this.showPage = function(obj){
        ...
        for (this.theadCount = 0; this.theadCount < tr.getElementsByTagName("TH").length; this.theadCount++) {
            ...
            if (!th.id.match('^diff_')) { // The Diff column is metadata and sorting/filtering operations do not work with it.
                var _th_menu = new YAHOO.widget.Menu(th.id + "_cm", {
                    container: th,
                    context: [th, "tl", "bl"],
                    lazyload: true,
                    itemdata: this.getMenuItems()
                });
        ...
        if (XNAT.app.tablePageLoadListeners) {
            $(XNAT.app.tablePageLoadListeners).each(function(i, listener){
                if ($.isFunction(listener)) {
                    listener(_div_table_id);
                }
            });
        }
    };
    ...
    this.renderOptions = function(obj1, obj2){
        ...
        this.optionMenu = new YAHOO.widget.MenuBar(this.div_table_id + "_options", {
            hidedelay: 300,
            maxheight: 200,
            ...
            if (this.getRoot() == this) {
                try {
                    ...
                    if (spec != null || this.en === "wrk:workflowData") {
                        submenuitems.push({
                            text: 'Email',
                            onclick: { fn: this.search.emailClick, scope: this.search }
                        });

                        submenuitems.push({
                            text: 'Save Search',
                            onclick: { fn: this.search.saveClick, scope: this.search }
                        });
                        submenuitems.push({
                            text: 'Save as New Search',
                            onclick: { fn: this.search.saveAsClick, scope: this.search }
                        });
                        submenuitems.push({
                            text: 'Show XML',
                            onclick: { fn: this.search.showXMLClick, scope: this.search }
                        });

                        if ((this.en.indexOf("SessionData") > -1)) {
                            submenuitems.push({
                                text: 'Download',
                                onclick: { fn: this.search.downloadClick, scope: this.search }
                            });
                        }
                        ...
                        if (this.search.obj.ID.startsWith("ss.")) {
                            submenuitems.push({
                                ...
                                onclick: { fn: this.search.deleteClick, scope: this.search }
                            });
                        }

                        submenuitems.push({
                            text: 'Edit Columns',
                            onclick: { fn: this.search.addColumnsClick, scope: this.search }
                        });
                        submenuitems.push({
                            text: 'Join to ...',
                            onclick: { fn: this.search.joinClick, scope: this.search }
                        });
            ...
            switch(task.index) {
                case 2:
                    ...
                    if (o.dt.config.sortedBy.key == colRow.getAttribute("name")) {
                        xmodal.message('Search Table', "Unable to remove a sorted column. Please sort by a different column and retry.");
                    }
                    else {
